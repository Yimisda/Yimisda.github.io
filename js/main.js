...
document.addEventListener("DOMContentLoaded", function () {
    ...
    if (savedThoughts) {
      try {
        ...
        thoughtsData.forEach((thought) => {
          if (!thought.id) thought.id = generateId();
        });
        saveThoughtsData(); // 更新存储
      } catch (e) {
  ...
  function renderThought(thought) {
    ...
    const isCurrentUser =
      !thought.isAnonymous || (thought.isAnonymous && thought.isCurrentUser);

    thoughtElem.innerHTML = `
            <div class="card-header">
                <h3 class="card-title">${
                  thought.isAnonymous ? "匿名用户" : "我"
                }</h3>
                <span class="card-date">${thought.date}</span>
                ${
                  isCurrentUser
                    ? '<div class="thought-actions"><button class="edit-btn">编辑</button><button class="delete-btn">删除</button></div>'
                    : ""
                }
            </div>
            <div class="card-body">
                ...
                ${
                  isCurrentUser
                    ? '<textarea class="edit-textarea" style="display: none;">' +
                      thought.content +
                      "</textarea>"
                    : ""
                }
                ${
                  isCurrentUser
                    ? '<div class="edit-actions" style="display: none;"><button class="save-btn">保存</button><button class="cancel-btn">取消</button></div>'
                    : ""
                }
            </div>
            <div class="card-footer">
                <div class="tag-container">
                    ...
                    <span class="like-counter" title="点赞数"><i class="like-icon">♥</i> <span class="like-count">${
                      thought.likes || 0
                    }</span></span>
                </div>
                <div class="action-links">
                    <a href="#" class="like-btn">${
                      thought.liked ? "取消点赞" : "点赞"
                    }</a>
  ...
  function renderAllThoughts() {
    ...
    // 按照日期降序排序树洞
    const sortedThoughts = thoughtsData.sort(
  ...
  document.addEventListener("click", function (e) {
    if (e.target.classList.contains("tag")) {
      ...
      if (e.target.classList.contains("tag-selected")) {
        ...
        // 筛选树洞
        document.querySelectorAll(".thought-card").forEach((card) => {
          const cardTags = Array.from(card.querySelectorAll(".tag")).map(
            (tag) => tag.textContent
          );
